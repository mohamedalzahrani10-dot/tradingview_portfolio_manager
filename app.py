...
# =========================================================
# DYNAMIC POSITION SIZING V2 + SESSION RISK MANAGER
# =========================================================
...
def next_datetime_for_time(now_dt, target_time):
...
def minutes_until_time(now_dt, target_time):
    return (next_datetime_for_time(now_dt, target_time) - now_dt).total_seconds() / 60.0
...
def add_history(state, event, details=None):
    ...
    state["history"].append({"ts": now_ts(), "date": info["date"], "session": info["session"], "event": event, "details": details or {}})
...
def get_reserved_cash(positions):
    return sum(get_position_reserved_value(pos) for pos in (positions or {}).values() if isinstance(pos, dict))


def get_available_cash_after_reserved(positions):
    reserved = get_reserved_cash(positions)
    return max(AVAILABLE_CASH - reserved, 0), reserved
...
def calculate_dynamic_quantity(data, positions):
    ...
    if not USE_DYNAMIC_POSITION_SIZING:
        return original_quantity, {"mode": "fixed_quantity_from_tradingview", "price_used": price, "quantity": original_quantity}
    available_after_reserved, reserved_cash = get_available_cash_after_reserved(positions)
    if AVAILABLE_CASH <= 0:
        return 0, {"mode": "dynamic_position_sizing_v2", "blocked": True, "reason": "AVAILABLE_CASH is not configured or <= 0", "available_cash": AVAILABLE_CASH, "price_used": price}
    if price <= 0:
        return 0, {"mode": "dynamic_position_sizing_v2", "blocked": True, "reason": "Missing valid price for sizing", "available_cash": AVAILABLE_CASH, "reserved_cash": round(reserved_cash, 2), "available_after_reserved": round(available_after_reserved, 2), "price_used": price}
    open_positions = len(positions)
    remaining_slots = max(MAX_POSITIONS - open_positions, 0)
    if remaining_slots <= 0:
        return 0, {"mode": "dynamic_position_sizing_v2", "blocked": True, "reason": "MAX_POSITIONS reached", "available_cash": AVAILABLE_CASH, "reserved_cash": round(reserved_cash, 2), "open_positions": open_positions, "max_positions": MAX_POSITIONS, "price_used": price}
    usable_cash = available_after_reserved * CASH_USAGE_PERCENT * (1 - SAFETY_BUFFER_PERCENT)
    ...
    if final_quantity < 1:
        return 0, {"mode": "dynamic_position_sizing_v2", "blocked": True, "reason": "Not enough cash to buy at least 1 share after reserved cash", "available_cash": AVAILABLE_CASH, "reserved_cash": round(reserved_cash, 2), "available_after_reserved": round(available_after_reserved, 2), "usable_cash": round(usable_cash, 2), "price_used": price, "original_quantity": original_quantity, "max_affordable_quantity": max_affordable_quantity}
    if trade_value < MIN_TRADE_VALUE:
        return 0, {"mode": "dynamic_position_sizing_v2", "blocked": True, "reason": "Trade value below MIN_TRADE_VALUE", "available_cash": AVAILABLE_CASH, "reserved_cash": round(reserved_cash, 2), "available_after_reserved": round(available_after_reserved, 2), "usable_cash": round(usable_cash, 2), "price_used": price, "quantity": final_quantity, "trade_value": round(trade_value, 2), "min_trade_value": MIN_TRADE_VALUE}
    return final_quantity, {"mode": "dynamic_position_sizing_v2", "available_cash": AVAILABLE_CASH, "reserved_cash": round(reserved_cash, 2), "available_after_reserved": round(available_after_reserved, 2), "usable_cash": round(usable_cash, 2), "cash_usage_percent": CASH_USAGE_PERCENT, "safety_buffer_percent": SAFETY_BUFFER_PERCENT, "open_positions": open_positions, "remaining_slots": remaining_slots, "price_used": price, "original_quantity": original_quantity, "max_affordable_quantity": max_affordable_quantity, "final_quantity": final_quantity, "estimated_trade_value": round(trade_value, 2)}
...
def validate_buy_against_session_rules(data):
    ...
    if BLOCK_BUY_NEAR_SESSION_END and info.get("near_session_end"):
        reasons.append(f"Near {info['session']} session end: {info['minutes_to_session_end']} minutes left")
    if info["session"] == "overnight" and not ALLOW_OVERNIGHT_HOLD:
        extras = get_extras(data)
        if not extras.get("timeAllowed", False):
...
def build_buy_payload(data, ticker, quantity, order_type):
    payload = {"ticker": ticker, "action": "buy", "quantity": quantity, "orderType": order_type}
...
def build_sell_payload(ticker, quantity):
    return {"ticker": ticker, "action": "sell", "quantity": quantity, "orderType": "market"}
...
def force_close_positions(reason="session_force_close"):
    ...
    for ticker, pos in list(positions.items()):
        ...
        if qty <= 0:
            continue
        ok = send_to_traderspost(build_sell_payload(ticker, qty))
        if ok:
    ...
    add_history(state, "FORCE_SESSION_CLOSE", {"reason": reason, "closed": closed, "failed": failed, "session": info})
    save_state(state)
...
def session_risk_worker():
    ...
    while True:
        try:
            ...
            if should_close:
                print("Session Risk Manager:", force_close_positions(reason=reason), flush=True)
...
def start_session_risk_background():
    if SESSION_RISK_MANAGER_ENABLED:
        threading.Thread(target=session_risk_worker, daemon=True).start()
    else:
...
def home():
    return jsonify({"ok": True, "service": "Portfolio Manager Running ✅", "version": "V2 Dynamic Sizing + Session Risk Manager", "session": get_session_info(), "max_positions": MAX_POSITIONS, "dynamic_position_sizing": USE_DYNAMIC_POSITION_SIZING, "session_risk_manager": SESSION_RISK_MANAGER_ENABLED, "available_cash_setting": AVAILABLE_CASH, "cash_usage_percent": CASH_USAGE_PERCENT, "safety_buffer_percent": SAFETY_BUFFER_PERCENT})
...
def webhook():
    ...
    if action in ["sell", "exit"]:
        if ticker not in positions:
            return jsonify({"ok": True, "decision": "ignored_exit_no_local_position", "ticker": ticker, "reason": "No local position found in portfolio_state"})
        exit_quantity = safe_int(positions.get(ticker, {}).get("quantity", quantity), quantity)
        ok = send_to_traderspost(build_sell_payload(ticker, exit_quantity))
        if ok:
            add_history(state, "EXIT_SENT", {"ticker": ticker, "quantity": exit_quantity, "reason": get_extras(data).get("reason", "tradingview_exit"), "source_action": action})
            positions.pop(ticker, None)
            state["positions"] = positions
            save_state(state)
        return jsonify({"ok": ok, "decision": "exit_sent", "ticker": ticker, "quantity": exit_quantity})
    ...
    if not flags_ok:
        ...
        save_state(state)
        return jsonify({"ok": True, "decision": "blocked_by_tradingview_safety_flags", "ticker": ticker, "score": score, "reasons": flag_reasons})

    session_ok, session_reasons = validate_buy_against_session_rules(data)
    if not session_ok:
        add_history(state, "BUY_BLOCKED_BY_SESSION_RULES", {"ticker": ticker, "reasons": session_reasons})
        save_state(state)
        return jsonify({"ok": True, "decision": "blocked_by_session_rules", "ticker": ticker, "score": score, "reasons": session_reasons, "session": get_session_info()})

    if ticker in positions:
        old_score = safe_float(positions[ticker].get("score", 0), 0)
        if score <= old_score:
            return jsonify({"ok": True, "decision": "ignored_duplicate_lower_score", "ticker": ticker, "old_score": old_score, "new_score": score})
        ...
        save_state(state)
        return jsonify({"ok": True, "decision": "updated_existing_position_score", "ticker": ticker, "score": score})
    ...
    if final_quantity < 1:
        ...
        save_state(state)
        return jsonify({"ok": True, "decision": "blocked_by_position_sizing", "ticker": ticker, "score": score, "sizing": sizing_info})

    if len(positions) < MAX_POSITIONS:
        payload = build_buy_payload(data, ticker, final_quantity, order_type)
        ok = send_to_traderspost(payload)
        if ok:
            positions[ticker] = {"score": score, "quantity": final_quantity, "entry_signal_price": signal_price, "entry_price": get_trade_price(data), "order_type": order_type, "limit_price": data.get("limitPrice"), "session": get_session_info()["session"], "sizing": sizing_info, "created_at": time.time(), "last_signal": data}
            state["positions"] = positions
            add_history(state, "BUY_SENT", {"ticker": ticker, "quantity": final_quantity, "score": score, "sizing": sizing_info})
            save_state(state)
        return jsonify({"ok": ok, "decision": "buy_sent", "ticker": ticker, "score": score, "signal_price": signal_price, "quantity": final_quantity, "sizing": sizing_info, "open_positions": len(positions)})

    weakest_ticker, weakest_data = min(positions.items(), key=lambda item: safe_float(item[1].get("score", 0), 0))
    weakest_score = safe_float(weakest_data.get("score", 0), 0)
    if score > weakest_score:
        weakest_quantity = safe_int(weakest_data.get("quantity", 1), 1)
        sell_ok = send_to_traderspost(build_sell_payload(weakest_ticker, weakest_quantity))
        if not sell_ok:
            return jsonify({"ok": False, "decision": "swap_failed_sell_rejected", "sold": weakest_ticker, "sold_score": weakest_score, "bought": ticker, "bought_score": score})
        positions.pop(weakest_ticker, None)
        state["positions"] = positions
        save_state(state)
        final_quantity, sizing_info = calculate_dynamic_quantity(data, positions)
        if final_quantity < 1:
            return jsonify({"ok": True, "decision": "swap_sell_done_but_buy_blocked_by_sizing", "sold": weakest_ticker, "bought": ticker, "sizing": sizing_info})
        buy_ok = send_to_traderspost(build_buy_payload(data, ticker, final_quantity, order_type))
        if buy_ok:
            positions[ticker] = {"score": score, "quantity": final_quantity, "entry_signal_price": signal_price, "entry_price": get_trade_price(data), "order_type": order_type, "limit_price": data.get("limitPrice"), "session": get_session_info()["session"], "sizing": sizing_info, "created_at": time.time(), "last_signal": data}
            state["positions"] = positions
            add_history(state, "SWAP", {"sold": weakest_ticker, "sold_score": weakest_score, "sold_quantity": weakest_quantity, "bought": ticker, "bought_score": score, "bought_quantity": final_quantity})
            save_state(state)
        return jsonify({"ok": sell_ok and buy_ok, "decision": "swap", "sold": weakest_ticker, "sold_score": weakest_score, "sold_quantity": weakest_quantity, "bought": ticker, "bought_score": score, "bought_quantity": final_quantity, "signal_price": signal_price, "sizing": sizing_info})

    return jsonify({"ok": True, "decision": "ignored_not_better_than_top_positions", "ticker": ticker, "score": score, "weakest_score": weakest_score})
...
def force_close_route():
    return jsonify(force_close_positions(reason="manual_force_close"))
...
def settings():
    ...
    positions = state.get("positions", {})
    available_after_reserved, reserved_cash = get_available_cash_after_reserved(positions)
    return jsonify({"MAX_POSITIONS": MAX_POSITIONS, "AVAILABLE_CASH": AVAILABLE_CASH, "RESERVED_CASH": round(reserved_cash, 2), "AVAILABLE_AFTER_RESERVED": round(available_after_reserved, 2), "CASH_USAGE_PERCENT": CASH_USAGE_PERCENT, "SAFETY_BUFFER_PERCENT": SAFETY_BUFFER_PERCENT, "MIN_TRADE_VALUE": MIN_TRADE_VALUE, "USE_DYNAMIC_POSITION_SIZING": USE_DYNAMIC_POSITION_SIZING, "RESPECT_TRADINGVIEW_SAFETY_FLAGS": RESPECT_TRADINGVIEW_SAFETY_FLAGS, "SESSION_RISK_MANAGER_ENABLED": SESSION_RISK_MANAGER_ENABLED, "BLOCK_BUY_NEAR_SESSION_END": BLOCK_BUY_NEAR_SESSION_END, "FORCE_CLOSE_BEFORE_SESSION_END": FORCE_CLOSE_BEFORE_SESSION_END, "SESSION_CLOSE_BUFFER_MINUTES": SESSION_CLOSE_BUFFER_MINUTES, "ALLOW_HOLD_BETWEEN_SESSIONS": ALLOW_HOLD_BETWEEN_SESSIONS, "ALLOW_OVERNIGHT_HOLD": ALLOW_OVERNIGHT_HOLD, "SESSION": get_session_info()})
